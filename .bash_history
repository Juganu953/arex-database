  try {
    ...
    console.log('- Plan Price:', userWithRelations?.currentPlan?.price ? '$' + userWithRelations.currentPlan.price : 'N/A')
    ...
    const tableCounts = await prisma.$queryRaw`
      SELECT 
        table_name,
        (SELECT COUNT(*) FROM information_schema.tables t2 WHERE t2.table_schema = t1.table_schema) as total_tables
      FROM information_schema.tables t1
      WHERE table_schema = 'public'
      ORDER BY table_name
    `
    console.log('Total tables in public schema:', tableCounts[0]?.total_tables || 0)
...
# Simple query to see what's in your database
export DATABASE_URL="postgres://d2e66ae8acdd11823deba06982dbd80926020310e48835e57f6864a4a40ae094:sk_XmezI6PgtiR_WFzq6BIfA@db.prisma.io:5432/postgres?sslmode=require"
node -e "
...
cat > package.json << 'EOF'
{
  ...
  "scripts": {
    ...
    "start": "node -r dotenv/config your-app.js"
  },
  "dependencies": {
...
npm install dotenv
# Now create a .env file if you haven't already
echo 'DATABASE_URL="postgres://d2e66ae8acdd11823deba06982dbd80926020310e48835e57f6864a4a40ae094:sk_XmezI6PgtiR_WFzq6BIfA@db.prisma.io:5432/postgres?sslmode=require"' > .env
# Test with the new setup
npm run db:test
# Remove the problematic test file
rm test-relationships.js
...
cat > .env << 'EOF'
DATABASE_URL="postgres://d2e66ae8acdd11823deba06982dbd80926020310e48835e57f6864a4a40ae094:sk_XmezI6PgtiR_WFzq6BIfA@db.prisma.io:5432/postgres?sslmode=require"
EOF

# Create a simple test to verify .env loading
cat > test-env.js << 'EOF'
...
async function quickStatus() {
  ...
  const tables = ['User', 'Plan', 'Lead', 'Transaction', 'BusinessProfile', 'AgentProfile', 'Session', 'Account']
  ...
  for (const table of tables) {
    try {
      ...
      console.log(table.padEnd(15)}: Table error`)
    }
  }
...
quickStatus().finally(() => prisma.\$disconnect())
"
